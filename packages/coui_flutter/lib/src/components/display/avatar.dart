...
class _AvatarState extends State<Avatar> {
  ...
  Widget _buildInitials(double borderRadius, BuildContext context) {
    final theme = Theme.of(context);
    final compTheme = ComponentTheme.maybeOf<AvatarTheme>(context);
...
class AvatarGroup extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < this.children.length; i += 1) {
      final avatar = this.children[i];
      final size = avatar.size ?? theme.scaling * 40;
      if (i == 0) {
        children.add(
          Positioned(left: currentX, top: currentY, child: avatar),
        );
...
class AvatarGroupClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    final widthDiff = size.width - prevAvatarSize;
    final heightDiff = size.height - prevAvatarSize;
