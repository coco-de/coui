...
class _NavigationBarState extends State<NavigationBar>
  ...
  Widget build(BuildContext context) {
    ...
    final parentPadding = styleValue(
      defaultValue:
          const EdgeInsets.symmetric(horizontal: 12, vertical: 8) * scaling,
    ...
    final rawChildren = wrapChildren(context, widget.children);
    List<Widget> children = <Widget>[];
    if (expands) {
      if (alignment == NavigationBarAlignment.spaceEvenly) {
        children.add(const Spacer());
        for (int i = 0; i < rawChildren.length; i += 1) {
...
double _startPadding(Axis direction, EdgeInsets padding) {
  return direction == Axis.vertical ? padding.top : padding.left;
}

double _endPadding(Axis direction, EdgeInsets padding) {
  return direction == Axis.vertical ? padding.bottom : padding.right;
}
    ...
    for (int i = 0; i < children.length; i += 1) {
      if (children[i].selectable) {
        newChildren[i] = Data.inherit(
          data: NavigationChildControlData(actualIndex: i, index: index),
          child: children[i],
        );
        index += 1;
      } else {
        newChildren[i] = Data.inherit(
          data: NavigationChildControlData(actualIndex: i),
          child: children[i],
        );
      }
    }
...
class _NavigationRailState extends State<NavigationRail>
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final scaling = theme.scaling;
    final parentPadding = widget.padding ??
        (const EdgeInsets.symmetric(horizontal: 12, vertical: 8) * scaling);
    final directionality = Directionality.of(context);
    final resolvedPadding = parentPadding.resolve(directionality);
    ...
    return RepaintBoundary(
      child: Data.inherit(
        data: NavigationControlData(
          ...
          parentPadding: resolvedPadding,
          selectedIndex: widget.index,
          spacing: widget.spacing ?? (scaling * 8),
        ),
        child: SurfaceBlur(
          surfaceBlur: widget.surfaceBlur,
          child: Container(
...
class _NavigationSidebarState extends State<NavigationSidebar>
    with NavigationContainerMixin {
  static BoxConstraints getDefaultConstraints(
      BuildContext context, ThemeData theme) {
    final scaling = theme.scaling;

    return BoxConstraints(maxWidth: scaling * 200, minWidth: scaling * 200);
  }

  static EdgeInsets _childPadding(Axis direction, EdgeInsets padding) {
    return direction == Axis.vertical
        ? EdgeInsets.only(left: padding.left, right: padding.right)
        : EdgeInsets.only(bottom: padding.bottom, top: padding.top);
  }
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final scaling = theme.scaling;
    final children = wrapChildren(context, widget.children);
    final parentPadding = widget.padding ??
        (const EdgeInsets.symmetric(horizontal: 12, vertical: 8) * scaling);
    final directionality = Directionality.of(context);
    final resolvedPadding = parentPadding.resolve(directionality);
...
enum NavigationLabelType { all, expanded, none, selected, tooltip }

enum NavigationLabelPosition { bottom, end, start, top }

enum NavigationLabelSize { large, small }

class NavigationChildControlData {
  const NavigationChildControlData({required this.actualIndex, this.index});
...
class NavigationGap extends StatelessWidget implements NavigationBarItem {
  ...
  Widget build(BuildContext context) {
    ...
    return data?.containerType == NavigationContainerType.sidebar
        ? buildSliver(context)
        : buildBox(context);
  }
}
...
class NavigationDivider extends StatelessWidget implements NavigationBarItem {
  const NavigationDivider({this.color, super.key, this.thickness});
        ...
        ? SliverToBoxAdapter(
            child: Padding(
              padding: direction == Axis.vertical
                  ? EdgeInsets.symmetric(vertical: scaling * 8)
                  : EdgeInsets.symmetric(horizontal: scaling * 8),
              child: child,
            ),
          )
        : Padding(
            padding: direction == Axis.vertical
                ? EdgeInsets.symmetric(vertical: scaling * 4)
                : EdgeInsets.symmetric(horizontal: scaling * 4),
            child: child,
          );
  }
}
...
class _NavigationItemState
  ...
  Widget buildBox(
    ...
    final direction = data?.direction ?? Axis.vertical;
    final index = childData?.index ?? widget.index;
    final isSelected = widget.selected ?? index == data?.selectedIndex;
    final parentIndex = childData?.index;
    final showLabel = labelType == NavigationLabelType.all ||
        (labelType == NavigationLabelType.selected && isSelected) ||
        (labelType == NavigationLabelType.expanded && data?.expanded ?? false);
    final style = widget.style ??
        (data?.containerType == NavigationContainerType.sidebar
            ? const ButtonStyle.ghost()
            : const ButtonStyle.ghost(density: ButtonDensity.icon));
    final selectedStyle = widget.selectedStyle ??
        (data?.containerType == NavigationContainerType.sidebar
...
abstract class _AbstractNavigationButtonState<
  ...
  Widget build(BuildContext context) {
    ...
    return labelType == NavigationLabelType.tooltip
        ? buildTooltip(context, data, childData)
        : _buildBox(context, data, childData);
  }
  ...
  Widget buildSliver(
    ...
    return labelType == NavigationLabelType.tooltip
        ? SliverToBoxAdapter(child: buildTooltip(context, data, childData))
        : SliverToBoxAdapter(child: _buildBox(context, data, childData));
  }
...
class _NavigationLabeled extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final direction = position == NavigationLabelPosition.top ||
            position == NavigationLabelPosition.bottom
        ? Axis.vertical
        : Axis.horizontal;
    final animatedSize = Hidden(
      direction: direction,
      hidden: !showLabel,
      keepCrossAxisSize:
...
class NavigationPadding extends StatelessWidget {
  const NavigationPadding({required this.child, super.key});
  ...
  Widget build(BuildContext context) {
    ...
    if (parentData != null && childData != null) {
      ...
      return Padding(
        padding: direction == Axis.vertical
            ? EdgeInsets.only(bottom: isLast ? 0 : gap, top: isFirst ? 0 : gap)
...
enum NavigationOverflow { clip, ellipsis, marquee, none }
...
class NavigationLabel extends StatelessWidget implements NavigationBarItem {
  ...
  final NavigationOverflow
      overflow; // these options are ignored in NavigationBar and NavigationRail
  final bool floating;
  ...
  Widget build(BuildContext context) {
    ...
    return data?.containerType == NavigationContainerType.sidebar
        ? buildSliver(context, data)
        : buildBox(context, data);
  }
  ...
  Widget buildBox(BuildContext context, NavigationControlData? data) {
    ...
    return DefaultTextStyle.merge(
      ...
      child: Container(
        ...
        padding: padding ?? EdgeInsets.symmetric(horizontal: scaling * 8),
      ...
      builder: (context, value, child) {
        return SliverPersistentHeader(
          delegate: _NavigationLabelDelegate(
            maxExtent: scaling * 48 * value,
            minExtent: scaling * 48 * value,
            child: GestureDetector(
              ...
              child: Container(
                ...
                padding:
                    padding ?? EdgeInsets.symmetric(horizontal: scaling * 16),
...
class _NavigationLabelDelegate extends SliverPersistentHeaderDelegate {
  const _NavigationLabelDelegate({
    required this.child,
...
class _NavigationLabelBackgroundPainter extends CustomPainter {
  const _NavigationLabelBackgroundPainter({
    required this.color,
    required this.direction,
...
class NavigationWidget extends StatelessWidget implements NavigationBarItem {
  ...
  Widget build(BuildContext context) {
    final data = Data.maybeOf<NavigationControlData>(context);
    final childData = Data.maybeOf<NavigationChildControlData>(context);
    final index = childData?.index ?? this.index;
